 ...
 * 用于读取ZIP常规加密文件流 <code>
 * 		<br/>ZipEntry entry;
 * 		<br/>FileInputStream fis = new FileInputStream(new File("test2.zip"));
 * 		<br/>ZipDecryptInputStream zdis = new ZipDecryptInputStream(fis, "password");
 * 		<br/>ZipInputStream zis = new ZipInputStream(zdis);
 * 		<br/>byte[] buf = new byte[2048];
 * 		<br/>while ((entry = zis.getNextEntry()) != null) {
 * 			<br/>if (entry.isDirectory()) {
 * 				<br/>System.out.println("error:   " + entry.getName() + "is skiped");
 * 				<br/>zis.closeEntry();
 * 				<br/>continue;
 * 			<br/>}
 * 			<br/>OutputStream outstr = new BufferedOutputStream(new FileOutputStream(new ziptest().GetFileName("D:/ziptest/test", entry.getName())));
 * 		 	<br/>int readLen = 0;
 * 			<br/>while (zis.available() > 0 && ((readLen = zis.read(buf, 0, BUFFER)) != -1)) {
 * 				<br/>outstr.write(buf, 0, readLen);
 * 			<br/>}
 * 			<br/>outstr.close();
 * 			<br/>zis.closeEntry();
 * 		<br/>}
 ...
 * 
 */
public class Lib_ZipDecryptInputStream extends InputStream {
	...
	public Lib_ZipDecryptInputStream(InputStream stream, String password) {
