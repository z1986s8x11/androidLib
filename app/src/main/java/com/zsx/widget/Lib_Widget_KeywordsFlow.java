 ...
 * 
 * @author zsx
 * @description set_Duration 飞入效果持续时间<br/>
 *              set_OnKeywordItemClickListener 设置监听<br/>
 *              clear_Keywords 清除关键字<br/>
 *              show_Keywords 调用显示 <br/>
 */
public class Lib_Widget_KeywordsFlow extends FrameLayout implements OnGlobalLayoutListener {
	...
	private KeywordOnItemClickListener keywordListener;
	...
	public Lib_Widget_KeywordsFlow(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init();
	}

	public Lib_Widget_KeywordsFlow(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public Lib_Widget_KeywordsFlow(Context context) {
		super(context);
		init();
	}
	...
	public void _setDuration(long duration) {
	...
	private boolean show() {
		if (width > 0 && height > 0 && vecKeywords != null && vecKeywords.size() > 0 && enableShow) {
			...
			for (int i = 0; i < size; i++) {
				...
				txt.setOnClickListener(new KeywordOnClick(keywordListener, txt, i));
				txt.setText(keyword);
	...
	private AnimationSet getAnimationSet(int[] xy, int xCenter, int yCenter, int type) {
		...
		if (type == OUTSIDE_TO_LOCATION) {
			...
			TranslateAnimation translate = new TranslateAnimation((xy[IDX_X] + (xy[IDX_TXT_LENGTH] >> 1) - xCenter) << 1, 0, (xy[IDX_Y] - yCenter) << 1, 0);
			...
			TranslateAnimation translate = new TranslateAnimation(0, (xy[IDX_X] + (xy[IDX_TXT_LENGTH] >> 1) - xCenter) << 1, 0, (xy[IDX_Y] - yCenter) << 1);
			animSet.addAnimation(translate);
	...
	public Vector<String> _getKeywords() {
		return vecKeywords;
	}

	public void _clearKeywords() {
		vecKeywords.clear();
		rubAllViews();
	}
	...
	public void _setOnKeywordItemClickListener(KeywordOnItemClickListener listener) {
		keywordListener = listener;
	}

	public interface KeywordOnItemClickListener {
		void onClick(TextView v, int index);
	}

	private class KeywordOnClick implements OnClickListener {
		private int index;
		private TextView t;
		private KeywordOnItemClickListener listener;

		public KeywordOnClick(KeywordOnItemClickListener listener, TextView t, int index) {
			this.index = index;
			this.t = t;
