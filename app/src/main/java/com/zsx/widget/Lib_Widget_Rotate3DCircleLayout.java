 ...
 * 实现子view绕y轴旋转的容器
 */
public class Lib_Widget_Rotate3DCircleLayout extends ViewGroup {
    ...
    public Lib_Widget_Rotate3DCircleLayout(Context context) {
        super(context);
        init(context);
    }

    public Lib_Widget_Rotate3DCircleLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public Lib_Widget_Rotate3DCircleLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            ViewHelper.setScaleX(child, ratio);
            ViewHelper.setScaleY(child, ratio);
...
//        int centerX = getMeasuredWidth() / 2 + (int) (Math.sin(radains) * radius + 0.5f); //每个圆的圆心的 x坐标
    ...
    private class CircleItem implements Comparable<CircleItem> {
